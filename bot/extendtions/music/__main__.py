...
class Music(commands.Cog):
    ...
    @commands.command(aliases=['come', 'cum'])
    ...
    @commands.command(pass_context=True,aliases=['p','pnext','pnow','ytmusic','playnext','playnow','play'])
    async def _wat(self, ctx, *, url=None):
        guildid = str(ctx.message.guild.id)
        callby = str(ctx.invoked_with)
        await self.join(ctx)
        voice = get(self.client.voice_clients, guild=ctx.guild)
        if url == None:
            await ctx.send('are we suppose to be playing something here?')
        else:
            if callby == 'playnow' or callby == 'pnow':
            ...
            if 'pnext' == callby or 'playnext' == callby:
                a = music.insert_song(guildid=guildid, url=url)
                if a != None:
                    await ctx.send(a)
                else:
    ...
    @commands.command(aliases=['current','song','info'])
    async def currentsonginfo(self, ctx, index=0):
        guildid = str(ctx.message.guild.id)
        # guildid = playlist.music(guildid=ctx.message.guild.id)
...
... and a {song["dislike"]}''')
    ...
    @commands.command(aliases=['is24/7'])
    ...
    async def clearlist(self, ctx):
        music.clearlist(guildid=str(ctx.message.guild.id))
        await ctx.send('gotchu, list cleared!')

    @commands.command()
    async def queue(self, ctx):
        msg, a = music.list_queue(guildid=str(ctx.message.guild.id))
    ...
    async def shuffle(self, ctx):
        music.shuffle(guildid=str(ctx.message.guild.id))
        await ctx.send(random.choice(botconfig['bot_reply']['shuffle']))

    @commands.command()
    async def lremove(self, ctx):
        music.removelast(guildid=str(ctx.message.guild.id))
    ...
    async def remove(self, ctx, songnum=None):
        if songnum == None:
            ...
            a, b = music.remove_song(guildid=str(ctx.message.guild.id),index=songnuma)
    ...
    async def warp(self, ctx):
        await ctx.send(music.sauce(guildid=str(ctx.message.guild.id)))

    @commands.command(aliases=['next','kingcrimson!','foward'])
    ...
    @commands.command(aliases=['songcount', 'total'])
    ...
    @commands.command(aliases=['bitethedust!','backward','previous'])
    ...
    @commands.command(aliases=['looplist'])
    ...
    @commands.command(aliases=['isloop'])
